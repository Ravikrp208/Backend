...
const ProductDetail = () => {
  ...
  const handleAddToCart = async () => {
    if (!activeVariant) {
      setErrorMsg("Please select all options first.");
      return;
    }
    setErrorMsg("");
    setSuccessMsg("");
  ...
  const handleBuyNow = async () => {
    ...
    try {
      ...
      console.error(err);
      setErrorMsg(err.response?.data?.message || "Failed to proceed to checkout.");
    }
  };
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-8 lg:px-16 xl:px-24 pt-12 lg:pt-16">
        <button
          onClick={() => navigate(-1)}
          className="mb-8 flex items-center gap-2 text-[10px] uppercase tracking-[0.2em] font-medium transition-colors hover:text-[#C9A96E]"
          style={{ color: "#7A6E63" }}
        >
          ← Return to Archive
        </button>
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-24 items-start">
          ...
          <div className="w-full lg:w-[30%] lg:sticky lg:top-24 flex flex-col pt-4">
            ...
            {successMsg && (
              <div className="mb-4 p-3 text-sm text-green-700 bg-green-50 border-l-2 border-green-500 font-medium tracking-wide">
                {successMsg}
              </div>
            )}
            {errorMsg && (
              <div className="mb-4 p-3 text-sm text-[#ba1a1a] bg-[#ffdad6] border-l-2 border-[#ba1a1a] font-medium tracking-wide">
                {errorMsg}
              </div>
            )}

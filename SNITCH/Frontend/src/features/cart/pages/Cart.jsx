...
const Cart = () => {
  ...
  const changeQty = (id, delta) => {
    setQuantities((prev) => {
      const currentQty = prev[id] ?? cartItems?.find((item) => item._id === id)?.quantity ?? 1;
      return {
        ...prev,
  ...
  if (!cartItems?.length) {
    return (
      <>
        ...
        >
          <div className="flex-1 flex flex-col items-center justify-center gap-6 pb-24 px-8 pt-16">
            <p
